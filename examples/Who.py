...
tl = TimeLine([
	...
	][::-1], bg_alpha=0.4, moments={5: lambda cursor: cursor.set_hp("-5"), 10: lambda cursor: cursor.set_hp("+15")}, on_death=lambda cursor: print('dead'))

area = Area([tl], cursor=cursor, minimize_windows=False, bg_func=mainloop)
area.on_tl_start = lambda area: area.set_alpha(0.2)
area.run(fps=120)